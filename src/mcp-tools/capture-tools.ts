...
const SearchRecordsSchema = z.object({
  server: z
    .string()
    .optional()
    .describe("Filter by specific server name. If not provided, searches across all servers."),

  session: z
    .string()
    .optional()
    .describe("Filter by specific session ID. Useful for analyzing a particular client interaction sequence."),

  method: z
    .string()
    .optional()
    .describe("Filter by MCP method name (e.g., 'tools/call', 'resources/read', 'prompts/get'). Supports partial matching."),

  messageType: z
    .enum(["request", "response", "notification", "error"])
    .optional()
    .describe("Filter by message type. 'request' shows incoming requests, 'response' shows server responses, 'notification' shows one-way messages, 'error' shows failed requests."),

  hasError: z
    .boolean()
    .optional()
    .describe("Filter to show only messages that resulted in errors (true) or exclude error messages (false). Omit to include both."),

  timeRange: z.object({
    start: z
      .string()
      .min(1, "Start time is required")
      .describe("Start time for search in ISO 8601 format (e.g., '2024-01-01T00:00:00Z'). Defaults to 24 hours ago."),
    end: z
      .string()
      .min(1, "End time is required")
      .describe("End time for search in ISO 8601 format. Defaults to current time.")
  })
    .optional()
    .describe("Time range to search within. Helps narrow results for large capture datasets."),
  ...
  limit: z
    ...
    .optional()
    .default(50)
    .describe("Maximum number of results to return (1-1000). Use pagination for large result sets."),
  ...
  offset: z
    ...
    .optional()
    .default(0)
    .describe("Number of results to skip for pagination. Combine with limit for paging through large datasets."),

  format: z
    .enum(["concise", "detailed"])
    .optional()
    .default("concise")
    .describe("Response format. 'concise' returns essential information for quick scanning, 'detailed' includes full request/response bodies and metadata."),
});
...
const AnalyzeSessionSchema = z.object({
  sessionId: z
    ...
    .describe("The session ID to analyze. Can be a full session ID or 'stateless' for one-off requests."),

  server: z
    .string()
    .optional()
    .describe("Optional server name to restrict analysis to. If not provided, analyzes the session across all servers."),

  includeTimeline: z
    .boolean()
    .optional()
    .default(true)
    .describe("Whether to include a chronological timeline of all session activities. Useful for understanding request flow."),

  includeMetrics: z
    .boolean()
    .optional()
    .default(true)
    .describe("Whether to include performance metrics (response times, error rates, method usage). Essential for performance analysis."),

  format: z
    .enum(["concise", "detailed"])
    .optional()
    .default("concise")
    .describe("Response detail level. 'concise' provides summary statistics and key insights, 'detailed' includes full interaction history."),
});
...
const GetServerStatsSchema = z.object({
  server: z
    ...
    .describe("Name of the server to generate statistics for. Must be a registered server in the gateway."),

  timeRange: z.object({
    start: z
      .string()
      .min(1, "Start time is required")
      .describe("Start time for statistics in ISO 8601 format. Defaults to 24 hours ago."),
    end: z
      .string()
      .min(1, "End time is required")
      .describe("End time for statistics in ISO 8601 format. Defaults to current time.")
  })
    .optional()
    .describe("Time range for statistics calculation. Shorter ranges provide more focused insights."),

  groupBy: z
    .enum(["hour", "day", "method"])
    .optional()
    .default("hour")
    .describe("How to group statistics. 'hour' for hourly breakdowns, 'day' for daily summaries, 'method' for per-method analysis."),

  includePerformance: z
    .boolean()
    .optional()
    .default(true)
    .describe("Whether to include performance metrics (response times, throughput). Critical for performance monitoring."),

  includeErrors: z
    .boolean()
    .optional()
    .default(true)
    .describe("Whether to include error analysis (error rates, common failures). Essential for reliability monitoring."),

  format: z
    .enum(["concise", "detailed"])
    .optional()
    .default("concise")
    .describe("Response format. 'concise' provides key metrics and alerts, 'detailed' includes comprehensive analysis and trends."),
});
 ...
 */
async function findCaptureFiles(storageDir: string, serverFilter?: string): Promise<CaptureFile[]> {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        for (const file of files) {
          ...
          const regex = new RegExp(`^(.+)-${serverNamePattern.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}-(.+)$`);
 ...
 */
function filterByTimeRange(records: CaptureRecord[], start?: string, end?: string): CaptureRecord[] {
 ...
 */
function filterByMethod(records: CaptureRecord[], method: string): CaptureRecord[] {
 ...
 */
function filterByMessageType(records: CaptureRecord[], messageType: 'request' | 'response' | 'notification' | 'error'): CaptureRecord[] {
  switch (messageType) {
    case 'request':
      return records.filter(record => !!record.request);
    case 'response':
      return records.filter(record => !!record.response && !record.response.error);
    case 'notification':
...
function calculateSessionMetrics(records: CaptureRecord[]): SessionMetrics {
  ...
  return {
    ...
    averageResponseTime,
    errorRate: responses.length > 0 ? (errors.length / responses.length) * 100 : 0,
    methodDistribution,
    duration,
    startTime,
    endTime,
 ...
 */
function formatConciseRecord(record: CaptureRecord, includePayload: boolean = false): string {
  const timestamp = new Date(record.timestamp).toLocaleString();
  const type = record.request ? (record.id === null ? 'NOTIFY' : 'REQ') : 'RESP';
 ...
 */
export function createCaptureTools(mcp: McpServer, registry: Registry, storageDir: string): void {
...
Results are sorted by timestamp (newest first) and include context like duration, status codes, and session information to help you quickly identify relevant interactions.`,
    ...
    handler: async (args) => {
      try {
        ...
        const paginatedRecords = allRecords.slice(args.offset || 0, (args.offset || 0) + (args.limit || 50));
        ...
        // Format results
        const recordsText = paginatedRecords.map((record, index) => {
          const number = (args.offset || 0) + index + 1;
          const formatted = formatConciseRecord(record, args.format === 'detailed');
          return `${number}. ${formatted}`;
        }).join('\n\n');

        const summary = `📊 Found ${totalResults} records (showing ${paginatedRecords.length})`;
        const pagination = totalResults > (args.limit || 50) ?
...
The tool provides actionable insights and recommendations based on the session analysis, helping you optimize both client usage and server configuration.`,
    ...
    handler: async (args) => {
      try {
        ...
        const captureFiles = await findCaptureFiles(storageDir, args.server);
        const sessionFiles = captureFiles.filter(file => file.sessionId === args.sessionId);
          ...
          return {
            content: [{
              ...
              text: `❌ Session '${args.sessionId}' not found.\n\n**Available sessions:**\n${
                captureFiles.slice(0, 10).map(f => `- ${f.sessionId} (${f.serverName})`).join('\n') || '(No sessions found)'
        ...
        for (const file of sessionFiles) {
          const records = await parseJsonlFile(file.path);
          sessionRecords.push(...records.filter(r => r.metadata.sessionId === args.sessionId));
        }
        ...
        const timeline = args.includeTimeline ? buildSessionTimeline(sessionRecords) : [];
        ...
        if (args.format === 'concise') {
          ...
          let insights = [];
          if (metrics.errorRate > 10) insights.push(`🔴 High error rate: ${metrics.errorRate.toFixed(1)}%`);
          if (metrics.averageResponseTime > 1000) insights.push(`🐌 Slow responses: ${metrics.averageResponseTime.toFixed(0)}ms avg`);
          if (exchanges.length === 0) insights.push(`⚠️ No completed exchanges found`);
...
💡 Use format=detailed for complete timeline and interaction history.`
          ...
          // Detailed format
          const timelineText = timeline.slice(0, 20).map((entry, i) => {
...
Try expanding the time range or check if the server has been active recently.`
        ...
        const errorRate = responses.length > 0 ? (errors.length / responses.length) * 100 : 0;
        ...
        const timeSeriesData: Array<{ timestamp: string; requests: number; errors: number; avgResponseTime: number }> = [];

        if (args.groupBy === 'hour' || args.groupBy === 'day') {
          const bucketSize = args.groupBy === 'hour' ? 60 * 60 * 1000 : 24 * 60 * 60 * 1000;
          const buckets = new Map<string, { requests: number; errors: number; responseTimes: number[] }>();
          ...
          allRecords.forEach(record => {
            ...
            if (!buckets.has(bucketKey)) {
              buckets.set(bucketKey, { requests: 0, errors: 0, responseTimes: [] });
            }
            ...
            if (record.response && !record.response.error && record.metadata.durationMs > 0) {
        ...
        const alerts = [];
        if (errorRate > 5) alerts.push(`🔴 High error rate: ${errorRate.toFixed(1)}%`);
        if (avgResponseTime > 1000) alerts.push(`🐌 Slow average response time: ${avgResponseTime.toFixed(0)}ms`);
        if (percentiles.p95 > 2000) alerts.push(`⚠️ High P95 latency: ${percentiles.p95.toFixed(0)}ms`);
